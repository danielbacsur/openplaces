...
OpenPlaces is a scraper: it drives Google Maps search through Playwright's headless Chromium and parses what the page renders into flat [`Place`](/openplaces/place) objects. There is no API key to configure and no bill — the only requirement is that the browser is available on the machine.
